...
// line → path に変換（pathLength アニメーションを使うため）
    ...
    >
      ...
      >
        ...
        <g mask="url(#vignette)">
          ...
          {/* ---- 中間六角形（回転） ---- */}
          <motion.polygon
            points={hexPoints(340, 360, 140)}
            fill="none"
            stroke="#d4a843"
            strokeWidth="0.5"
            strokeOpacity="0.10"
            initial={{ opacity: 0 }}
            animate={prefersReduced ? { opacity: 1 } : { opacity: 1, rotate: 360 }}
            transition={
              prefersReduced
                ? { duration: 1.5 }
                : {
            ...
            style={{ originX: "340px", originY: "360px" }}
          />

          {/* ---- 内側六角形（逆回転） ---- */}
          <motion.polygon
            points={hexPoints(340, 360, 80)}
            fill="none"
            stroke="#d4a843"
            strokeWidth="0.5"
            strokeOpacity="0.14"
          ...
          {/* ---- 対角線（line → path に変更して pathLength が動くように） ---- */}
          ...
          {/* ---- 中心スポーク（line → path に変更） ---- */}
          {DOTS.slice(0, 6).map((dot, i) => (
            <motion.path
              key={`spoke-${i}`}
              d={linePath(340, 360, dot.x, dot.y)}
              fill="none"
              stroke="#d4a843"
              strokeWidth="0.3"
              strokeOpacity="0.06"
          ...
          {/* ---- 頂点のドット（scale → opacity のみに変更して安定化） ---- */}
          {DOTS.map((dot, i) => (
            <motion.circle
              key={`dot-${i}`}
              cx={dot.x}
